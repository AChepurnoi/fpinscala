...
object MyModule {
  ...
  def fib(n: Int): Int = {
    def nextFib(left: Int, right: Int, c: Int): Int = {
      if (c == 0) right
      else nextFib(right, right + left, c - 1)
    }

    n match {
      case 0 => 0
      case 1 => 1
      case _ => nextFib(0, 1, n - 1)
    }
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = (as zip as.drop(1))
    .map{ x => gt(x._1, x._2)}
    .reduce(_ && _)
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    println("Sorted: " + isSorted(Array(1, 2, 3, 4), (l: Int, r: Int) => l < r))
    println("Not Sorted: " + isSorted(Array(1, 2, 4, 4), (l: Int, r: Int) => l < r))
